...
client.on("message", message => {
    ...
    if(command === "give") {
      if(!message.author.id === message.guild.owner) return message.reply("انت مش شيدي -_- متقدرش تعمل كده");
      const user = message.mentions.users.first() || client.users.get(args[0]);
      if(!user) return message.reply("لازم تمنشن الشخص اولا لتاخذ الادي");
...
client.on('message', message => {
  ...
  if (command == "ban") {
               if(!message.channel.guild) return message.reply('** :Wrong: هذا الامر للاداره فقط**');
  ...
  .bannable) return message.reply(`لا تستطيع تبنيد الاداره :Wrong: `);
...
client.on('message', message => {
  ...
  if(!message.guild.member(message.author).hasPermission("KICK_MEMBERS")) return message.reply("**لا تملك صلاحيه للكيك :Wrong: **");
  if(!message.guild.member(client.user).hasPermission("KICK_MEMBERS")) return message.reply("**لا املك صلاحيه للكيك :Wrong: **");
  let user = message.mentions.users.first();
  let reason = message.content.split(" ").slice(2).join(" ");
  ...
  if (!message.guild.member(user)
  .kickable) return message.reply("**لا يمكنك عمل كيك للاداره**");
...
if(!message.guild.member(client.user).hasPermission("MANAGE_CHANNELS")) return message.reply(`**I Dont Have Permission For That !`);
...
if(!message.guild.member(client.user).hasPermission("MANAGE_ROLES_OR_PERMISSIONS")) return message.reply(`**I Dont Have Permission For That !`);
...
client.on('message', message => {
    ...
    .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=507676841990029313&permissions=0&scope=bot`)  // حط اي دي بوتك
